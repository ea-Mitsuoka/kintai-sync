...
本システムは、Slackの特定チャンネル（部署チャンネル）におけるユーザーの投稿を検知し、そのメッセージ内容を解析して、以下の5つの外部システム・コンポーポネントへ情報を同期・操作する範囲を対象とする。
...
- **【NFR-4.2】初期構築・運用コマンドの自動化 (Makefile)**
  ...
  - **インフラ破棄の分離**: アプリケーションのみを削除する `make destroy` と、プロジェクトを完全に更地に戻す `make destroy-all` を使い分け可能とする。
  - 複雑な `gcloud` コマンドや `terraform` コマンドのフラグ管理を Makefile 内に隠蔽する。
  - **プリフライトチェック (`make check`)**: 権限やAPI有効化状態の自動検証機能。
  - **クリーンアップ (`make prune`)**: 不要な一時ファイルやディレクトリの自動一掃機能。
  - **秘密情報の安全な登録 (`make register-secrets`, `make register-user`)**: シークレットの対話型登録機能。

- **【NFR-4.3】一元的なログ・秘匿情報の管理**

  - `make logs` による Cloud Run 実行ログのクイックアクセス。
  - `make secrets` による Secret Manager への登録状況確認。
...
| **Vertex AI** | Gemini 3 Flash Preview (1.2万回) | ~$0.50 | 非常に低価格 |
