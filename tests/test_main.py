...
def test_worker_endpoint(
    ...
    mock_slack_inst.get_user_info.return_value = {
...
def test_worker_none_type(
    ...
    mock_parser_inst = mock_parser.return_value
    mock_parser_inst.parse.return_value = AttendanceInfo(
        target_dates=[], attendance_type="none", original_message="Just chatting"
    )
    ...
    assert response.status_code == 200
    assert response.get_json()["status"] == "success"
    # Verify Slack reply was called (for early exit message)
    mock_slack.return_value.reply_to_thread.assert_called_once()
