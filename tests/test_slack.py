...
def test_post_attendance_report_failure(slack_manager):
    slack_manager.mock_client.chat_postMessage.side_effect = SlackApiError("Error", {"error": "channel_not_found"})
...
def test_reply_to_thread_failure(slack_manager):
    slack_manager.mock_client.chat_postMessage.side_effect = SlackApiError("Error", {"error": "thread_not_found"})
...
def test_set_user_status_failure(mock_web_client, slack_manager):
    mock_user_client = mock_web_client.return_value
    mock_user_client.users_profile_set.side_effect = SlackApiError("Error", {"error": "invalid_auth"})
    
    slack_manager.set_user_status("user-token", "Working Remotely", ":house:")

def test_get_user_info_success(slack_manager):
    slack_manager.mock_client.users_info.return_value = {"user": {"id": "U1", "name": "testuser"}}
...
def test_get_user_info_failure(slack_manager):
    slack_manager.mock_client.users_info.side_effect = SlackApiError("Error", {"error": "user_not_found"})
